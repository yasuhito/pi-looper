...
目的は、単に便利に開発することではありません。実際のリポジトリで「issue を作るだけで実装ループが進む」運用を続けることで、公開前に安全性、設定しやすさ、失敗時の止まり方、prompt と決定論的スクリプトの境界を検証します。

## 基本方針

最初から自動マージまで有効にしません。段階的に dogfooding します。
...
1. **Phase 1: 実装 PR 作成まで**
   ...
   - agent-ready issue を拾い、Herdr worktree の Pi worker に実装させる。
   - coordinator が検証して PR を作る。
   - PR レビューとマージは人間が行う。
2. **Phase 2: レビュー自動化を試す**
   - safety controls が入ってから `generic-pr-reviewer` を有効にする。
   - 最初は自動マージを禁止し、人間確認に渡す運用で試す。
3. **Phase 3: 条件付き自動マージ**
   - dry-run、manual approval、自動マージ無効化、失敗時停止条件が揃ってから検討する。
...
- agent-ready issue の契約が十分に具体的か
- worker が `AGENTS.md` と README を読んで実装できるか
- `checkCommand` が適切に失敗を検出するか
- PR 作成までの handoff が読みやすいか
- Herdr worktree の作成・完了検出・片付けが安定しているか

## 推奨ローカル設定

`projects.json` はローカル設定なので、リポジトリにコミットしません。ローカル path、対象 repo、`autoMerge` などの rollout 判断が入るため、公開 package には `projects.example.json` だけを含めます。例:
...
## 最初に dogfood する issue

最初の題材は safety controls がよいです。
...
## Agent Brief
pi-looper の dogfooding を安全に進めるため、PR reviewer automation の自動マージを設定で止められるようにする。

## What to build
- project config に `autoMerge` または同等の safety flag を追加する。
- `generic-pr-reviewer` prompt に、その flag が無効の場合は merge せず `ready-for-human` に渡す方針を反映する。
- 既定値は安全側に倒す。
...
- `npm pack --dry-run` にローカル設定や cache が含まれない。

## Out of scope
- tmux runner の追加。
- review worker の全面的な再設計。
```

## 標準検証

pi-looper 自体の変更や package 内容を確認するときは、次を標準検証として使います。
...
`npm pack --dry-run` では、`extensions/pi-looper/projects.json`、cache、worktree artifact、`node_modules/`、Python bytecode が package に含まれていないことも確認します。
...
- worker が `<promise>BLOCKED: ...</promise>` を返した
- `checkCommand` が失敗した
- PR が draft のまま
- 外部レビューや CI が失敗した
- prompt が想定外に push / merge / label 操作を worker に許している
