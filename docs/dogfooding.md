...
既存の legacy `~/.pi/agent/pi-looper/projects.json` を使っている場合は、必要に応じて `~/.pi/agent/deadloop/projects.json` へコピーし、この設定例と同じ短い `issue-coordinator.*` / `pr-reviewer.*` のファイル名へ更新してください。
